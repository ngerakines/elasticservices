...
public class DefaultServiceRegistry implements ServiceRegistry {
    ...
    @Override
    public synchronized void bindTransportToService(final ServiceProto.ServiceRef serviceRef, final Transport transport) {
        ...
        if (transportServiceBindings.containsEntry(transportRef, serviceRef)) {
            LOGGER.error("Attempted to bind transport {} to service-ref {} when binding already exists.",
                    transport.getRef(),
                    serviceRef);
            return;
        }
    ...
    public synchronized List<ServiceProto.ServiceRef> getServices(final ServiceProto.ComponentRef componentRef) {
        return Ordering.from(new ServiceRefComparator())
                .sortedCopy(ImmutableList.copyOf(Iterables.filter(serviceRefs,
    ...
    @Override
    public synchronized List<ServiceProto.ServiceRef> getServices(final String site, final String cluster, final String id) {
    ...
    private Optional<? extends Message> composeMessage(
        ...
        if (contentType != null) {
            final Optional<String> classNameOptional = CollectionUtils.firstAttributeValue(contentType.getAttributeList(),
                    "class");
            if (classNameOptional.isPresent()) {
                final String className = classNameOptional.get();
                final List<MessageFactory> messageFactories = ImmutableList.copyOf(this.messageFactories
                        .get(className));
                for (final MessageFactory messageFactory : messageFactories) {
                    final Optional<? extends Message> fabricMessageOptional = messageFactory.get(messageController,
                            rawMessage);
    ...
    private static class SiteIdServiceRefsFilter implements Predicate<ServiceProto.ServiceRef> {
        ...
        @Override
        public boolean apply(@Nullable final ServiceProto.ServiceRef input) {
            return input != null && site.equals(input.getComponentRef()
